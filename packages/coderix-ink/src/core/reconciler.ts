...
export function isDebugRepaintsEnabled(): boolean {
  if (debugRepaints === undefined) {
    debugRepaints = process.env.CODERIX_DEBUG_REPAINTS === '1'
  }
...
// Set CODERIX_COMMIT_LOG=1 to enable debug logging to stderr.
const COMMIT_LOG = process.env.CODERIX_COMMIT_LOG
let _commits = 0
let _lastLog = 0
let _lastCommitAt = 0
let _maxGapMs = 0
