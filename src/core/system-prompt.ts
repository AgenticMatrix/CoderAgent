...
export class SystemPromptAssembler {
  ...
  private getCoordinatorPersona(): string {
    return [
      ...
      '',
      '=== BACKGROUND AGENT WORKFLOW (CRITICAL) ===',
      '',
      'After spawning background sub-agents:',
      '1. Tell the user what you launched and why.',
      '2. Call the Sleep tool to wait for results. Do NOT poll with TaskGet.',
      '   Sleep will return early as soon as a sub-agent completes.',
      '3. When Sleep returns, you will receive a <background-agent-notifications>',
      '   block containing <task-notification> elements for completed agents.',
      '4. IMMEDIATELY process the results: summarize findings, present to the user.',
      '   Do NOT wait for the user to ask "how is it going" — be proactive.',
      '5. If some agents are still running, call Sleep again to wait for the rest.',
      '',
      'When reading agent results:',
      '- The notification includes an <output_path>. Use Read on that path',
      '  to get the full result if the inline summary is truncated.',
      '- The inline <result> in the notification may be truncated;',
      '  the file at <output_path> contains the complete output.',
      '',
      'Only use TaskGet for debugging or if the user specifically asks.',
      'Use TaskStop only to cancel a misbehaving sub-agent.',
    ].join('\n');
  }
